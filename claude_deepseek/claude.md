...
# 变电站施工流程引导程序
## 技术要求
1. 使用 Python + PyQt5 开发桌面GUI程序，最终打包为单EXE可执行文件，双击即可运行，无需用户安装额外依赖
2. 所有数据100%本地存储，不上传任何外部服务器，符合工程资料保密要求
3. 全程中文界面、中文注释、中文提示，禁用任何技术术语
## 核心红线规则
1. 绝对禁止删除、移动、修改用户的原始照片和扫描件，仅可复制副本进行归档
2. 所有生成的业务文件优先输出Word、Excel格式，方便打印和提交
3. 界面按钮、字体优先放大设计，适配非技术人员操作
## 功能优先级
1. 施工节点管理 + 多级到期提醒 + 前置准备清单
2. 照片/扫描件按节点时间自动分类归档
3. 进度款申请表生成 + 附件自动打包 + 收款台账
## 归档规范
