...
export function CurrentNodeCard({ node, daysLeft }: CurrentNodeProps) {
  if (!node) {
    return (
      <Card>
        <h3 className="text-lg font-semibold text-gray-800 mb-2">当前节点</h3>
        <p className="text-gray-400">暂无进行中的节点</p>
      </Card>
    );
  }
    ...
    <Card className="border-l-4 border-l-blue-500">
      ...
      <div className="grid grid-cols-2 gap-4 mb-3 text-sm">
        <div>
          ...
          <span className="text-gray-700 ml-1">{node.startDate || "未设置"}</span>
        </div>
        <div>
          <span className="text-gray-400">截止：</span>
          <span className={`ml-1 font-medium ${daysLeft <= 3 ? "text-red-600" : daysLeft <= 7 ? "text-orange-600" : "text-gray-700"}`}>
            {node.endDate || "未设置"}
