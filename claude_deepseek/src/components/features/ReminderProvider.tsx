  ...
  async function checkReminders() {
    try {
      ...
      for (const reminder of json.data) {
        const key = `${reminder.nodeId}-${reminder.type}`;
        if (notifiedRef.current.has(key)) continue;
        notifiedRef.current.add(key);

        const message = REMINDER_MESSAGES[reminder.type]?.(reminder.nodeName, reminder.daysDiff)
          || reminder.nodeName;
        ...
        if (typeof Notification !== "undefined" && Notification.permission === "granted") {
          try {
            new Notification("变电施工项目提醒", { body: message });
          } catch {
            // 忽略通知失败
          }
        }
